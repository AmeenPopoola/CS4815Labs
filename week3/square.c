...
void mouse(int btn, int state, int x, int y)
{
    ...
    if(btn==GLUT_RIGHT_BUTTON && state==GLUT_DOWN) size=5;
    if(btn==GLUT_MIDDLE_BUTTON && state==GLUT_DOWN) size=10;
    if(btn==GLUT_LEFT_BUTTON && state==GLUT_DOWN) size=15;
}

void quitSquare(unsigned char key , int x , int y){
  if(key == 'k' || key == 'Q')
    exit(0);
}
